...
public class ArticleDao {
    ...
    private final String QUERTYPAGEBYCATEGORY = "select * from t_article where _categoryid=? limit ?, ?";
    ...
    public List<Article> queryByCategory(final String categoryId) {
        return jdbcTemplate.query(QUERYBYCATEGORY, new Object[]{categoryId}, new RowMapper<Article>() {
            ...
            public Article mapRow(ResultSet resultSet, int i) throws SQLException {
                ...
                article.setCreateDate(resultSet.getDate("_createDate"));
                article.setUpdateDate(resultSet.getDate("_updateDate"));
                article.setCategoryID(UUID.fromString(categoryId));
                article.setGurl(resultSet.getString("_gurl"));
                article.setViewCount(resultSet.getInt("_viewCount"));
                article.setZpic(resultSet.getString("_zpic"));
                article.setHomePic(resultSet.getString("_homePic"));
    ...
    public Page<Article> queryPageByCategory(final String categoryId,
                                             final Pageable query) {

        //获取总行
        String sql = "select count(*) from t_article where _categoryid=?";
        int total = jdbcTemplate.queryForObject(sql, new Object[]{categoryId}, Integer.class);

        List<Article> list=jdbcTemplate.query(QUERTYPAGEBYCATEGORY, new Object[]{categoryId,query.getOffset(),query.getPageSize()}, new RowMapper<Article>() {
                    @Override
                    public Article mapRow(ResultSet resultSet, int i) throws SQLException {
                        Article article = new Article();
                        article.setId(UUID.fromString(resultSet.getString("_id")));
        ...
        return new PageImpl<Article>(list,query,total);
    }
