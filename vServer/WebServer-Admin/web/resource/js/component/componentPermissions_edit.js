...
Ext.onReady(function () {
    ...
    var mergeStore = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            extraParams: roleParam,
            url: rootPath + 'component/main/'+pid+'/permissions',
            reader: {
                type: 'json',
                root: 'entity'
            },
        ...
        autoLoad: true,
        listeners: {
            load: function (t,r,s,e) {
                if(s){
                   var len= r.length;
                    for(var i=0;i<len;i++){
                        if(r[i].get('type')==1)
                            haveRoleStore.add({name:r[i].get('name'),permission:tempp[r[i].get('permission')]});
                        else
                            haveAccountStore.add({account:r[i].get('name'),permission:tempp[r[i].get('permission')]});
                    }
                }
            }
        }
