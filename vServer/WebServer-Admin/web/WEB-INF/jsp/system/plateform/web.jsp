...
<div class="maincontent" style="margin-left: 0px;">
    ...
    <div id="webdiv" style="margin: 5px;">
        <form method="post" action="system/category/web/update" onsubmit="return CheckSubmit();" id="formupdate">
            <div class="form_default">
                <fieldset style="padding: 0 10px 10px 10px;margin:10px;">
                    <p style="display: none;">
                        <label for="category">PId:</label>
                        <input style="width: 350px;background-color:#F7F7F7;" type="text" id="category" name="category" value="${web.category}" readonly="true" class="redonly"/>
                    </p>

                    <p>
                        <label>系统名称:</label><b>${web.name}</b>
                        <%--<input style="width: 350px;" type="text" id="cname" name="cname" value="${web.getName()}" readonly="true" class="redonly"/>--%>
                    </p>
                    <p>
                        <label>修改日期:</label><span><fmt:formatDate value="${web.updateTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
                        <%--&lt;%&ndash;<input style="width: 350px;" type="text" id="cupdateTime" name="cupdateTime" value="${web.getUpdateTime()}" readonly="true" class="redonly"/>&ndash;%&gt;--%>
                    </p>
                    <p>
                        <label>创建人 : </label><span>${web.createUser}</span>
                        <%--&lt;%&ndash;<input style="width: 350px;" type="text" id="ccreateUser" name="ccreateUser" value="${web.getCreateUser()}" readonly="true" class="redonly"/>&ndash;%&gt;--%>
                    </p>
                    <p>
                        <label for="ckey">Key :</label>
                        <input style="width: 350px;" type="text" id="ckey" name="ckey" value="${web.key}" />
                    </p>

                    <p>
                        <label for="cstartResource">起始页挂载资源 :</label>
                        <input type="text" id="cstartResource" name="cstartResource"  style="width: 350px;"
                               value="${web.startResource}" class="inputNessCss" />
                    </p>

                    <p>
                        <label for="cstartscript">开始脚本 :</label>
                        <textarea id="cstartscript" name="cstartscript" rows="10" cols="128"
                                >${web.startScript}</textarea>
                    </p>

                    <p>
                        <label for="cendscript">结束脚本 :</label>
                        <textarea id="cendscript" name="cendscript" rows="10" cols="128"
                                >${web.endScript}</textarea>
                    </p>
