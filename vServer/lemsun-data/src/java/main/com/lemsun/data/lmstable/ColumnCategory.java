...
public final class ColumnCategory {
   ...
   public static final int Unkonw = 0;
    ...
    /**
     * 单据号, 一张表中只能存在一个单据列. 如表格具备了单据列. 那么就可以不需要 Code 列
     * 如果有单据列. 那么可以不出现的 FormSec 序列 列
     */
    public static final int FormNO = 16;

    /**
     * 单据的子项目序列号
     */
    public static final int FormSec = 17;
    ...
    public static int getSQLType(int columnCategory)
    {
        ...
        return c == ColumnCategory.KEY || c == ColumnCategory.ADATE || c == ColumnCategory.CODE ? Types.VARCHAR
                ...
                : c == ColumnCategory.INT || c == ColumnCategory.FormNO || c == ColumnCategory.FormSec ? Types.INTEGER
